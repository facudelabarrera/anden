        ...
        <div className="min-w-0 flex flex-1 flex-col gap-8 lg:gap-12">
          <StaggerParent className="flex flex-col gap-4">
            {STATS_DATA.map((stat, i) => (
              <StaggerChild
                key={i}
                className="flex flex-col gap-4 rounded-xl border border-orange-brand bg-cream-light p-4 sm:flex-row sm:items-start"
              >
                <p className="font-alfarn text-2xl text-blue-brand sm:min-w-[120px] lg:min-w-[160px] lg:text-[30px]">
                  {'counter' in stat ? (
                ...
                <p className="font-sans text-xs uppercase leading-[14px] text-blue-brand">
                  {stat.description}
                </p>
              </StaggerChild>
            ))}
          ...
          <Reveal delay={0.2}>
            <div className="border-l-2 border-blue-brand py-2 pl-4">
              <p className="font-sans text-base text-brown-dark">
                PRÓXIMAS JURISDICCIONES
              </p>
              <p
                className="mt-4 font-body text-sm font-light text-brown-dark"
                style={{ fontVariationSettings: "'SOFT' 0, 'WONK' 1" }}
              >
                El modelo es replicable en cualquier jurisdicción con marco de ZEE
                vigente. Colombia, Uruguay y Chile en evaluación.
              </p>
            </div>
          </Reveal>
        </div>
