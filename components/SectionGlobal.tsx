            ...
            <StaggerChild key={region.location}>
              <TiltCard className="grid h-full grid-rows-[auto_1fr_auto] gap-6 rounded-2xl border border-cream/40 bg-[#8894ff] p-4 sm:gap-8">
                ...
                <div className="grid min-h-[44px] grid-cols-[auto_1fr] items-start gap-x-3 gap-y-2">
                  <span className="rounded-xl border border-blue-brand bg-cream-light px-4 py-2 font-sans text-sm font-medium uppercase text-blue-brand md:text-base">
                    {region.amount}
                  </span>
                  <span
                    className="min-w-0 self-center justify-self-end text-right font-body text-[11px] font-light italic uppercase leading-tight text-cream-light md:text-[13px] font-variation-normal"
                  >
                    {region.location}
                  </span>
                </div>
                ...
                <div className="flex min-h-0 flex-col justify-start">
                  <p
                    className="font-body text-sm font-light italic text-blue-brand md:text-base font-variation-normal"
                  >
                    {region.category}
                  </p>
                </div>
                {/* Bottom badge */}
                <div
                  className="mt-auto rounded-lg px-2.5 py-1"
                  style={{ backgroundColor: 'rgba(247, 243, 231, 0.5)' }}
                >
                  <p className="text-center font-sans text-sm font-bold uppercase text-blue-brand md:text-base">
                    {region.badge}
                  </p>
                </div>
              </TiltCard>
